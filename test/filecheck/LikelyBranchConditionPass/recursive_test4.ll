...
; should be optimized both branch - recursive call inside of true basic block 
...
; CHECK-LABEL:   if.end:
; CHECK:         %4 = load i32, i32* %l.addr, align 4
; CHECK-NEXT:    %5 = load i32, i32* %r.addr, align 4
; CHECK-NEXT:    %add1 = add nsw i32 %4, %5
; CHECK-NEXT:    %div = sdiv i32 %add1, 2
; CHECK-NEXT:    store i32 %div, i32* %m, align 4
; CHECK-NEXT:    %6 = load i32, i32* %k.addr, align 4
; CHECK-NEXT:    %7 = load i32, i32* %m, align 4
; CHECK-NEXT:    %cmp2 = icmp sgt i32 %6, %7
; CHECK-NEXT:    br i1 %cmp2, label %if.end4, label %if.then3
if.end:
...
; CHECK-LABEL:   if.end4:
; CHECK:         %12 = load i32, i32* %m, align 4
; CHECK-NEXT:    %13 = load i32, i32* %k.addr, align 4
; CHECK-NEXT:    %cmp5 = icmp sge i32 %12, %13
; CHECK-NEXT:    br i1 %cmp5, label %if.end8, label %if.then6
if.end4:
